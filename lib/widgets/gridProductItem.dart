    ...
    return ClipRect(
        child: GridTile(
            child: GestureDetector(
                onTap: () => Modular.to.pushNamed(RT_ITEM_DETAILS + _product.get_id()),
                child: Image.network(_product.get_imageUrl(), fit: BoxFit.cover)),
            footer: GridTileBar(
                leading: IconButton(
                    icon: Observer(
                        builder: (BuildContext context) =>
                            _gridProductItemStore.favoriteStatus ?? _product.get_isFavorite()
                                ? IOS_ICO_FAV
                                : IOS_ICO_NOFAV),
                    onPressed: () {
                      _gridProductItemStore.toggleFavoriteStatus(_product.get_id());
                    },
                ...
                trailing: IconButton(
                    ...
                    onPressed: () {
                      ...
                      FlushNotifier(DONE, _product.get_title() + MSG_CART_ADD, FLSBR_TIME, context)
                          .withButton(UNDO, () => _cartStore.undoAddProductInTheCart(_product));
                    },
                    color: Theme.of(context).accentColor),
